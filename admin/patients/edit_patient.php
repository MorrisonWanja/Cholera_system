...
if(isset($_POST["edit_pat_action"])) {
  ...
  if($err_pat_name == '' && $err_no == ''  && $err_mail == ''  && $err_county == ''  && $err_loc == ''  && $err_doctor == '') {
	  
	
   if($_POST["edit_pat_action"] == "Edit Patient") {

		$latitude = clean_text($_POST["latitude"]);
		$longitude = clean_text($_POST["longitude"]);
		$patient_id = clean_text($_POST["patient_id"]);
	
		$patient->editPatient($patient_id,$patient_name,$phone_number,$email,$county,$doctor_id,$location,$latitude,$longitude);
	
		$message = '<div class="alert alert-warning">Patient edited successfully</div>';
