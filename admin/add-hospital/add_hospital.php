...
if(isset($_POST["add_hos_action"])) {
  ...
  if($err_hos_name == '' && $err_county == ''  && $err_tel == ''  && $err_cat_no == ''  && $err_loc == '') {
   ...
   if($_POST["add_hos_action"] == "Add Hospital") {
		$latitude = clean_text($_POST["latitude"]);
		$longitude = clean_text($_POST["longitude"]);
			
		$hospital->addHospital($hospital_name,$hospital_category,$tel_number,$county,$location,$latitude,$longitude);
	
		$message = '<div class="alert alert-success">Hospital added successfully</div>';
